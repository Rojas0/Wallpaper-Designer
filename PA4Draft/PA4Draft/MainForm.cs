...
namespace PA4Draft
{
    public partial class MainForm : Form
    {
        ...
        private void HatchBrush_Click(object sender, EventArgs e)
        {
            ...
            if (d != DialogResult.OK)
                return;
            shapes[shapeList.SelectedIndex].fillBrush = new HatchBrush(p.hStyle,p.pickedColor,p.pickedColor1);
            updateTileDesign();
            updateShapeList(shapeList.SelectedIndex);
        }
        ...
        private void TextureBrush_Click(object sender, EventArgs e)
        {
            ...
            if (d != DialogResult.OK)
                return;
            TextureBrush texture = new TextureBrush(new Bitmap(p.file));
            texture.WrapMode = p.tile;
            shapes[shapeList.SelectedIndex].fillBrush = texture;
            updateTileDesign();
            updateShapeList(shapeList.SelectedIndex);
        }
        ...
        private void LinearGradientBrush_Click(object sender, EventArgs e)
        {
            ...
            if (d != DialogResult.OK)
                return;
            shapes[shapeList.SelectedIndex].fillBrush = new LinearGradientBrush(p.rect, p.pickedColor, p.pickedColor1, p.linMode);
            updateTileDesign();
            updateShapeList(shapeList.SelectedIndex);
        }
